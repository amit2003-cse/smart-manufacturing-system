...
const ProductionReport = () => {
  const [selectedItem, setSelectedItem] = useState(() => sessionStorage.getItem('rpt_prod_item') || null);
  const [selectedBatch, setSelectedBatch] = useState(() => sessionStorage.getItem('rpt_prod_batch') || null);
  const [displayData, setDisplayData] = useState(() => {
    const saved = sessionStorage.getItem('rpt_prod_data');
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    sessionStorage.setItem('rpt_prod_item', selectedItem || '');
    sessionStorage.setItem('rpt_prod_batch', selectedBatch || '');
    sessionStorage.setItem('rpt_prod_data', JSON.stringify(displayData));
  ...
  const getProductionStatus = (itemCode, batchCode, boxNo) => {
    const unit = unitBoxesDB.find(
      b => b.itemCode === itemCode && b.batchCode === batchCode && b.boxNo === boxNo
    );
    if (!unit) return { text: 'Not Done', color: '#ef4444', bg: '#fef2f2', unit: null };
    if (!unit.isScanned) return { text: 'Pending for Scan', color: '#f59e0b', bg: '#fffbeb', unit };
    return { text: 'Done', color: '#10b981', bg: '#f0fdf4', unit };
  };
  ...
  const handleSearch = () => {
    ...
    for (let i = 1; i <= itemInfo.boxCapacity; i++) {
      const status = getProductionStatus(selectedItem, selectedBatch, i);
      ...
      reportData.push({
        ...
        boxNo: i,
        barcode: status.unit ? status.unit.barcode : `${selectedItem}-${selectedBatch}-U${sequence}`,
        status: status.text,
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['S.No', 'Item Code', 'Batch Code', 'Unit Box', 'Barcode', 'Status'];
    let csvContent = headers.join(",") + "\n";
    displayData.forEach((row) => {
      csvContent += [row.sNo, row.itemCode, row.batchCode, row.boxNo, row.barcode, `"${row.status}"`].join(",") + "\n";
    });
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `Production_Report_${selectedItem}_${selectedBatch}.csv`);
  ...
  const summary = useMemo(() => {
    ...
    const pending = displayData.filter(d => d.status === 'Pending for Scan').length;
  ...
  return (
    <div className="page-container fade-in">
      <div className="page-header">
        <h2 className="page-title">Production Report</h2>
        <p className="help-text">
          View the production status of all unit boxes for a selected Item and Batch. Track generation and scanning progress.
        </p>
      </div>
        ...
        <>
          ...
          <div className="report-summary-row">
            ...
            <div className="summary-card" style={{ borderLeftColor: '#f59e0b' }}>
              ...
              <span className="summary-label">Pending for Scan</span>
            </div>
          ...
          <div className="grid-card card">
            <div className="section-header">
              <h4 style={{ margin: 0, color: '#1e293b' }}>
                ...
                Production Status ({displayData.length} Units)
              </h4>
      ...
      {displayData.length === 0 && (
        <div className="no-data-msg" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', flexDirection: 'column', gap: '16px' }}>
          ...
          <span>Select Item and Batch, then click Search to generate the report.</span>
        </div>
      )}
