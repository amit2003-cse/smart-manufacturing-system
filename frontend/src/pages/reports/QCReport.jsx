...
const QCReport = () => {
  const [selectedItem, setSelectedItem] = useState(() => sessionStorage.getItem('rpt_qc_item') || null);
  const [selectedBatch, setSelectedBatch] = useState(() => sessionStorage.getItem('rpt_qc_batch') || null);
  const [displayData, setDisplayData] = useState(() => {
    const saved = sessionStorage.getItem('rpt_qc_data');
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    sessionStorage.setItem('rpt_qc_item', selectedItem || '');
    sessionStorage.setItem('rpt_qc_batch', selectedBatch || '');
    sessionStorage.setItem('rpt_qc_data', JSON.stringify(displayData));
  ...
  const getQCStatus = (itemCode, batchCode, boxNo) => {
    // First find the unit box to get its barcode
    ...
    // Check QC status using barcode
    const qcRecord = qcDB.find(q => q.barcode === unit.barcode);
    if (!qcRecord) return { text: 'Not Done', color: '#ef4444', bg: '#fef2f2' };
    if (qcRecord.status === 'PENDING_QC') return { text: 'Pending for QC Decision', color: '#f59e0b', bg: '#fffbeb' };
    if (qcRecord.status === 'APPROVED') return { text: 'Done (Approved)', color: '#10b981', bg: '#f0fdf4' };
    if (qcRecord.status === 'REJECTED') return { text: 'Done (Rejected)', color: '#dc2626', bg: '#fef2f2' };
    return { text: 'Done', color: '#10b981', bg: '#f0fdf4' };
  };
  ...
  const handleSearch = () => {
    ...
    for (let i = 1; i <= itemInfo.boxCapacity; i++) {
      const status = getQCStatus(selectedItem, selectedBatch, i);
      reportData.push({
        sNo: i,
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `QC_Report_${selectedItem}_${selectedBatch}.csv`);
  ...
  const summary = useMemo(() => {
    const done = displayData.filter(d => d.status.startsWith('Done')).length;
    const pending = displayData.filter(d => d.status === 'Pending for QC Decision').length;
  ...
  return (
    <div className="page-container fade-in">
      <div className="page-header">
        <h2 className="page-title">QC Report</h2>
        <p className="help-text">
          View Quality Control status for all unit boxes. Track which units have been inspected, approved, or are still pending.
        </p>
      </div>
        ...
        <>
          ...
          <div className="report-summary-row">
            ...
            <div className="summary-card" style={{ borderLeftColor: '#f59e0b' }}>
              ...
              <span className="summary-label">Pending QC</span>
            </div>
          ...
          <div className="grid-card card">
            <div className="section-header">
              <h4 style={{ margin: 0, color: '#1e293b' }}>
                ...
                QC Status ({displayData.length} Units)
              </h4>
      ...
      {displayData.length === 0 && (
        <div className="no-data-msg" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', flexDirection: 'column', gap: '16px' }}>
          ...
          <span>Select Item and Batch, then click Search to generate the QC report.</span>
        </div>
      )}
